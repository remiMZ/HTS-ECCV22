...
def save_checkpoint(args, backbone, model, train_log, optimizer, global_task_accout, tag):
...
if __name__ =='__main__':
    ...
    parser = argparse.ArgumentParser('Relation Network')
    parser.add_argument('--model-name', type=str, default='relation_HTS_DA', help='Name of the model.')
    ...
    parser.add_argument('--backbone', type=str, default='conv4', help='The type of model backbone.')
    ...
    args.record_folder = './{}_{}_{}_{}way_{}shot_{}'.format(args.train_data, args.backbone, args.pretext_tasks, str(args.num_ways), str(args.num_shots), cur_time)
    ...
    model = RelationNetwork(args.out_channels, args.feature_h)
    ...
    with tqdm(train_dataloader, total=int(args.train_tasks/args.batch_tasks), initial=int(global_task_count/args.batch_tasks)) as pbar:
        for train_batch_i, train_batch in enumerate(pbar):
            ...
            support_features, support_features_shape = backbone(support_inputs)
            query_features, query_features_shape = backbone(query_inputs)
            ...
            support_h = model(support_data, fsl_support_inputs.device, is_lstm=True)
            query_h = model(query_data, fsl_query_inputs.device, is_lstm=True)
            ...
            prototypes = get_prototypes(support_embeddings, fsl_support_targets, train_dataset.num_classes_per_task)    

            class_features = prototypes.view(-1, args.num_ways, *support_features_shape[1:])
            query_features = query_embeddings.view(-1, args.num_ways*args.test_shots, *query_features_shape[1:])

            class_features = class_features.unsqueeze(1).repeat(1, args.num_ways*args.test_shots, 1, 1, 1, 1)
            query_features = query_features.unsqueeze(2).repeat(1, 1, args.num_ways, 1, 1, 1)

            relation_pairs = torch.cat((class_features, query_features), 3)    
            relations_scores = model(relation_pairs, fsl_support_inputs.device)

            train_loss = relation_loss(relations_scores, fsl_query_targets)
            ...
            with torch.no_grad():
                train_acc = count_acc(relations_scores, fsl_query_targets)
            ...
            if global_task_count % args.validation_tasks == 0:
                ...
                with torch.no_grad():
                    with tqdm(val_dataloader, total=int(args.val_tasks/args.batch_tasks)) as pbar:
                        for val_batch_i, val_batch in enumerate(pbar, 1):
                            ...
                            prototypes = get_prototypes(support_embeddings, fsl_support_targets, val_dataset.num_classes_per_task)   
                            ...
                            val_loss = relation_loss(relations_scores, fsl_query_targets)
                            val_acc = count_acc(relations_scores, fsl_query_targets)
                ...
                if val_acc_mean > train_log['max_acc']:
                    ...
                    save_model(backbone, model, args, tag='max_acc')
                ...
                save_checkpoint(args, backbone, model, train_log, optimizer, global_task_count, tag='max_acc')
                # save tensorboard writer
    ...
    with torch.no_grad():
        with tqdm(test_dataloader, total=int(args.test_tasks/args.batch_tasks)) as pbar:
            for test_batch_i, test_batch in enumerate(pbar, 1): 
                ...
                test_loss = relation_loss(relations_scores, fsl_query_targets)
                test_acc = count_acc(relations_scores, fsl_query_targets)
