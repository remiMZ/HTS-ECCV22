...
def relation_loss(predict, targets):
    relation_target = torch.zeros(*predict.shape, device=predict.device).scatter_(2, targets.unsqueeze(-1), 1)
    
    return F.mse_loss(predict, relation_target)
