...
from model import Backbone, RelationNetwork, Classifier
from tree_data import get_tree, convert_tree_to_tensors
from utils import multi_augmented_dataset, get_dataset, relation_loss, get_loss, get_ssl_classes
...
if __name__ =='__main__':
    ...
    parser.add_argument('--model-name', type=str, default='relation_HTS_SSL', help='Name of the model.')
    ...
    # set reproducibility
    if args.reproduce:
        set_reproducibility(args.seed)# set reproducibility
    ...
    for ssl_task in args.pretext_tasks:
        classifier = Classifier(args.out_channels, args.feature_h, num_classes[ssl_task])
    ...
    with tqdm(train_dataloader, total=int(args.train_tasks/args.batch_tasks), initial=int(global_task_count/args.batch_tasks)) as pbar:
        for train_batch_i, train_batch in enumerate(pbar):
            ...
            prototypes = get_prototypes(fsl_support_embeddings, fsl_support_targets, 
                                        train_dataset.num_classes_per_task)    

            class_features = prototypes.view(-1, args.num_ways, *support_features_shape[1:])
            fsl_query_features = fsl_query_embeddings.view(-1, args.num_ways*args.test_shots, *query_features_shape[1:])

            class_features = class_features.unsqueeze(1).repeat(1, args.num_ways*args.test_shots, 1, 1, 1, 1)
            fsl_query_features = fsl_query_features.unsqueeze(2).repeat(1, 1, args.num_ways, 1, 1, 1)

            relation_pairs = torch.cat((class_features, fsl_query_features), 3)    
            ...
            with torch.no_grad():
                ...
                del support_features, query_features, support_h, query_h, ssl_query_targets
                del support_data, query_data, support_tree, query_tree, ssl_support_targets
                del fsl_support_embeddings, fsl_query_embeddings, ssl_support_embeddings, ssl_query_embeddings
