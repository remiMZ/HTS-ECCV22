...
from model import TreeLSTM, Backbone, GraphNeuralNetwork
from tree_data import get_tree, convert_tree_to_tensors
from utils import multi_augmented_dataset, get_dataset, get_loss, get_one_hot_targets
from global_utils import Averager, Mean_confidence_interval, get_outputs_c_h, count_acc, set_reproducibility

def save_model(model, treelstm, backbone, args, tag):
...
def save_checkpoint(args, model, treelstm, backbone, train_log, optimizer, global_task_accout, tag):
...
if __name__ =='__main__':
    ...
    parser.add_argument('--model-name', type=str, default='GNN_HTS_DA', help='Name of the model.')
    ...
    parser.add_argument('--pretext-tasks', nargs='+', type=str, default=['rotation1'], 
        ...
        for train_batch_i, train_batch in enumerate(pbar):
            ...
            if global_task_count % args.validation_tasks == 0:
                ...
                if val_acc_mean > train_log['max_acc']:
                    ...
                    save_model(model, treelstm, backbone, args, tag='max_acc')
                ...
                save_checkpoint(args, model, treelstm, backbone, train_log, optimizer, global_task_count, tag='max_acc')
                # save tensorboard writer
