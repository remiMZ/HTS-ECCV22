...
from model import TreeLSTM, Backbone, GraphNeuralNetwork, Classifier
from tree_data import get_tree, convert_tree_to_tensors
from utils import multi_augmented_dataset, get_dataset, get_loss, get_one_hot_targets, get_ssl_classes
from global_utils import Averager, Mean_confidence_interval, get_outputs_c_h, count_acc, set_reproducibility

def save_model(model, treelstm, backbone, classifier_all, args, tag):
...
def save_checkpoint(args, model, treelstm, backbone, classifier_all, train_log, optimizer, classifier_optimizer, global_task_accout, tag):
...
if __name__ =='__main__':
    ...
    parser.add_argument('--model-name', type=str, default='GNN_HTS_SSL', help='Name of the model.')
    ...
    parser.add_argument('--pretext-tasks', nargs='+', type=str, default=['rotation'], 
    ...
    model = GraphNeuralNetwork(args.out_channels, args.num_ways, hid_features=96, J=1)
    treelstm = TreeLSTM(args.out_channels)
    classifier_all = []
    for ssl_task in args.pretext_tasks:
        classifier = Classifier(args.out_channels, num_classes[ssl_task])
    ...
    optimizer = torch.optim.Adam([
                        ...
                        {'params':treelstm.parameters()}], lr=args.lr, weight_decay=0.0005)
    ...
    if args.resume and args.resume_folder is not None:
        ...
        treelstm.load_state_dict(state['treelstm'])
        for i in range(len(args.pretext_tasks)):
    ...
    with tqdm(train_dataloader, total=int(args.train_tasks/args.batch_tasks), initial=int(global_task_count/args.batch_tasks)) as pbar:
        for train_batch_i, train_batch in enumerate(pbar):
            ...
            support_h = treelstm(support_data, fsl_support_inputs.device)
            query_h = treelstm(query_data, fsl_query_inputs.device)
            ...
            support_one_hot_targets = get_one_hot_targets(fsl_support_targets, train_dataset.num_classes_per_task)
            predictions = model(support_embeddings, support_one_hot_targets, query_embeddings)   

            train_loss = get_loss(predictions, fsl_query_targets)
            ...
            with torch.no_grad():
                train_acc = count_acc(predictions, fsl_query_targets)
                ...
                del support_features, query_features, support_embeddings, query_embeddings
                del support_data, query_data, support_tree, query_tree, support_h, query_h
            ...
            if global_task_count % args.validation_tasks == 0:
                ...
                with torch.no_grad():
                    with tqdm(val_dataloader, total=int(args.val_tasks/args.batch_tasks)) as pbar:
                        for val_batch_i, val_batch in enumerate(pbar, 1):
                            ...
                            support_one_hot_targets = get_one_hot_targets(fsl_support_targets, val_dataset.num_classes_per_task)
                            predictions = model(support_embeddings, support_one_hot_targets, query_embeddings)   

                            val_loss = get_loss(predictions, fsl_query_targets)

                            val_ssl_loss = torch.tensor(0., device=fsl_support_inputs.device)
                            for i in range(len(args.pretext_tasks)):
                                classifier_all[i].eval()
                                ssl_support_mask = support_data['node_order'] == len(args.pretext_tasks)-(i+1)
                                ssl_support_embeddings = support_h[ssl_support_mask,:].view(args.batch_tasks, int(support_shape[i+1]/args.batch_tasks), -1)
                                ssl_query_mask = query_data['node_order'] == len(args.pretext_tasks)-(i+1)
                                ssl_query_embeddings = query_h[ssl_query_mask,:].view(args.batch_tasks, int(query_shape[i+1]/args.batch_tasks), -1)
                            ...
                            val_loss_main = train_loss + args.ssl_trade * val_ssl_loss
                            
                            val_acc = count_acc(predictions, fsl_query_targets)
                ...
                if val_acc_mean > train_log['max_acc']:
                    ...
                    save_model(model, treelstm, backbone, classifier_all, args, tag='max_acc')
                ...
                save_checkpoint(args, model, treelstm, backbone, classifier_all, train_log, optimizer, classifier_optimizer, global_task_count, tag='max_acc')
                # save tensorboard writer
    ...
    with torch.no_grad():
        with tqdm(test_dataloader, total=int(args.test_tasks/args.batch_tasks)) as pbar:
            for test_batch_i, test_batch in enumerate(pbar, 1): 
                ...
                support_one_hot_targets = get_one_hot_targets(fsl_support_targets, test_dataset.num_classes_per_task)
                predictions = model(support_embeddings, support_one_hot_targets, query_embeddings)   
                
                test_loss = get_loss(predictions, fsl_query_targets)
                ...
                test_acc = count_acc(predictions, fsl_query_targets)
