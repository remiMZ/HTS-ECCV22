...
from utils import multi_augmented_dataset, get_dataset
...
def save_checkpoint(args, model, backbone, train_log, optimizer, global_task_accout, tag):
...
if __name__ =='__main__':
    ...
    parser.add_argument('--model-name', type=str, default='MatchingNet_HTS_DA', help='Name of the model.')
    ...
    parser.add_argument('--pretext-tasks', nargs='+', type=str, default=['rotation4'], help='The type of ssl pretext-tasks.')
    ...
    with tqdm(train_dataloader, total=int(args.train_tasks/args.batch_tasks), initial=int(global_task_count/args.batch_tasks)) as pbar:
        for train_batch_i, train_batch in enumerate(pbar):
            ...
            train_loss = matching_loss(support_embeddings, fsl_support_targets, query_embeddings, fsl_query_targets, args.num_ways)
            ...
            with torch.no_grad():
                train_log_probas = matching_log_probas(support_embeddings, fsl_support_targets, 
                    query_embeddings, args.num_ways)
                ...
                del support_shape, support_inputs, query_shape, query_inputs 
                del support_h, query_h, ssl_ways
                ...
                with torch.no_grad():
                    with tqdm(val_dataloader, total=int(args.val_tasks/args.batch_tasks)) as pbar:
                        for val_batch_i, val_batch in enumerate(pbar, 1):
                            ...
                            val_loss = matching_loss(support_embeddings, fsl_support_targets,
                                query_embeddings, fsl_query_targets, args.num_ways)
            
                            val_log_probas = matching_log_probas(support_embeddings, fsl_support_targets, 
                                query_embeddings, args.num_ways)
    ...
    with torch.no_grad():
        with tqdm(test_dataloader, total=int(args.test_tasks/args.batch_tasks)) as pbar:
            for test_batch_i, test_batch in enumerate(pbar, 1): 
                ...
                test_loss = matching_loss(support_embeddings, fsl_support_targets,
                    query_embeddings, fsl_query_targets, args.num_ways)
            
                test_log_probas = matching_log_probas(support_embeddings, fsl_support_targets, 
                    query_embeddings, args.num_ways)
