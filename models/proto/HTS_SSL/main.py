...
from utils import multi_augmented_dataset, get_dataset, get_accuracy, get_loss, get_ssl_classes
...
if __name__ =='__main__':
    ...
    parser.add_argument('--model-name', type=str, default='ProtoNet_HTS_SSL', help='Name of the model.')
   
    parser.add_argument('--data-folder', type=str, default='./datasets',
        help='Path to the folder the data is downloaded to.')   
    parser.add_argument('--train-data', type=str, default= 'miniimagenet', choices=['cub', 'miniimagenet',
        'tieredimagenet','cifar_fs'], help='Name of the dataset.')
    parser.add_argument('--test-data', type=str, default= 'miniimagenet', choices=['cub', 'miniimagenet',
        'tieredimagenet','cifar_fs'], help='Name of the dataset.')
    parser.add_argument('--num-shots', type=int, default=1, choices=[1, 5, 10],
    ...
    with tqdm(train_dataloader, total=int(args.train_tasks/args.batch_tasks), initial=int(global_task_count/args.batch_tasks)) as pbar:
        for train_batch_i, train_batch in enumerate(pbar, int(global_task_count / args.batch_tasks)+1):
            ...
            fsl_support_root_mask = support_data['node_order'] == len(args.pretext_tasks)
            fsl_support_embeddings = support_h[fsl_support_root_mask,:].view(*fsl_support_inputs.shape[:2], -1)
            fsl_query_root_mask = query_data['node_order'] == len(args.pretext_tasks)
            fsl_query_embeddings = query_h[fsl_query_root_mask,:].view(*fsl_query_inputs.shape[:2], -1)
    
            prototypes = get_prototypes(fsl_support_embeddings, fsl_support_targets,
                train_dataset.num_classes_per_task)
            train_loss = prototypical_loss(prototypes, fsl_query_embeddings, fsl_query_targets)
            ...
            with torch.no_grad():
                train_acc = get_accuracy(prototypes, fsl_query_embeddings, fsl_query_targets)
                ...
                with torch.no_grad():
                    with tqdm(val_dataloader, total=int(args.val_tasks/args.batch_tasks)) as pbar:
                        for val_batch_i, val_batch in enumerate(pbar, 1):
                            ...
                            val_loss = prototypical_loss(prototypes, fsl_query_embeddings, fsl_query_targets)
                            ...
                            val_acc = get_accuracy(prototypes, fsl_query_embeddings, fsl_query_targets)
    ...
    with torch.no_grad():
        with tqdm(test_dataloader, total=int(args.test_tasks/args.batch_tasks)) as pbar:
            for test_batch_i, test_batch in enumerate(pbar, 1): 
                ...
                test_loss = prototypical_loss(prototypes, fsl_query_embeddings, fsl_query_targets)
                ...
                test_acc = get_accuracy(prototypes, fsl_query_embeddings, fsl_query_targets)
